...
async function generarLeccion(btn) {
  ...
  // Opciones extra para lección completa
  const ollamaIPEl = document.getElementById('l-ollama-ip');
  const ollamaIP   = ollamaIPEl ? ollamaIPEl.value.trim() || 'localhost:11434' : 'localhost:11434';
  const ollamaModel = (aiAvailable && aiModels.length > 0)
    ? (document.getElementById('ai-model-sel')?.value || 'deepseek-r1:1.5b')
    : 'deepseek-r1:1.5b';

  // Armar preguntas para el quiz embebido (Fase 2)
  let quizPreguntas = [];
  if (selectedFmt === 'leccion-completa') {
    const banco = PREGUNTAS[selectedMateria]?.[selectedTema] || [];
    const bancoExt = (typeof BANCO_EXT !== 'undefined') ? (BANCO_EXT[selectedMateria]?.[selectedTema] || []) : [];
    const raw = normalizarPreguntas(mezclar([...banco, ...bancoExt]));
    quizPreguntas = mezclar(raw).slice(0, 10);
  }

  let html = '';
  if (selectedFmt === 'leccion-completa') html = generarLeccionCompleta(leccion, selectedMateria,
    Object.assign({}, opts, { ollamaIP, ollamaModel, preguntas: quizPreguntas, lives: 3, timeQuiz: 30 }));
  else if (selectedFmt === 'slides')          html = generarSlides(leccion, selectedMateria, opts);
